...
alter the Rollout-Simulation parse of the MCTS algorithm by instead of doing
...
of an MCTS. We utilize a baseline function $B(b_i)$ dependent on the block
$b_i$. During the training we gather the rewards of each block, we then
compute the average and multiply it by the discount factor $\gamma$ with
exponent dependant on the state $s^{b_i}_{i_t}$ as per $G_i$. E.g. the first
state of an episode the exponent would be $T$ and at the last one $1$.
\newline
The action taken at the end of an MCTS is determined by the MCTS improved policy
$\pi^{\text{MCTS}}$. The $Q$ values are estimated through the MCTS and
parametrized policy $\pi_\theta$ search. The full reinforcement learning
algorithm algorithm in pseudo-code is the following
\begin{algorithm}[H]
...
$100$ iterations. The marginal exchange rates are pulled from the block range
