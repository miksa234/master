\chapter{Modeling Constant Function Market Makers (CFMM) \label{chap: cfmms}}
On top of blockchain technology a new form of financial markets have found
its roots, they are called Decentralized Exchanges (DEXs). As in any
financial market, DEXs allow agents to trade assets, in this case
cryptocurrencies (digital assets) which we will refer throughout as an asset.
Thorough a decentralized ledger stored copied on multiple computers
(blockchain) the asset ownership is being tracked through a storage slot of a
smart contract. All Agents holding an asset have a form of entry in the
storage slot, equivalent to a database entry in the assets' smart contract
with their wallet address and amount of asset. In this chapter we go through
the underlying principles of DEXs and their properties by mathematical
modeling and optimization to arrive at optimal trades and ultimately model
arbitrage scenarios. In recent years a lot of groundwork on modeling has been
done in \cite{cfmms_basics} and on optimal trades in
...
reserve of asset $i \in \{1,\ldots,n\}$. The reserves are the defined
liquidity of the DEX and lie in its portfolio. There are two underlying ways
an agent can interact with the DEX:
\begin{itemize}
    \item Change liquidity
    \item Trade
\end{itemize}
We will only look the trading interaction which lies in the scope of the aim
of the thesis.
\newline
...
\end{align}
In the case we are trading only a one to one of these assets then we have
...
is determined by ticks $\{p(i)\}_{i \in \mathbb{Z} }$ with the rate
usually set to $p(i) = 1.0001^{i}$, where $p(i)$ is the so-called marginal
...
exchange rate is in the this specific tick range, then the liquidity of the
tick range is used for the trade. The liquidity providers specify an upper
and a lower bound $(p(U), p(L)]$ and provide liquidity on depth $\tilde{k} =
\varphi(y)$. They earn the fee of the trades, which is split by the weighted
amount of liquidity provided in the tick range instead of on the whole
trading curve.
...
acceptance condition. It is an reassuring that the trade can approximated by
the marginal exchange rates which is the usual intuition. In fact as we will
later see by concavity of the trade function this will in an inequality.
\section{Swaps}
...
function is non-negative and increasing because of $\varphi$, but it unlike
...
continuously differentiable function in $\mathbb{R}$ with an initial guess
$x_0$ near a root $a$ the iteration
\begin{align}
...
solution of an optimization problem that maximizes the change in the traders'
portfolio $x -\tilde{x}$ through a utility function. The agent prefers
a change in holdings that maximize a utility function $U: \mathbb{R}^{n}  \to
\mathbb{R} \cup \{-\infty\} $. For two trades $(x, \tilde{x}$ and $(x',
...
utility function is chosen to be increasing and concave, and thereby
differentiable. The trade that maximizes the utility can be framed as a
constrained optimization problem, through the trade acceptance condition
\begin{align}
...
\end{align}
We would prefer the problem in equation \ref{eq: gen_trade} to be
convex, then it can be solved efficiently fast using convex methods. The
feasible set of the optimization problem is
\begin{align}
...
solution. Furthermore the relaxed problem fulfills Slater CQ by choosing the
choice of $\tilde{x}=0$ we can find a $x \in \mathbb{R}^{n}$ such that by the
monotonicity assumptions on $\varphi$ we have
\begin{align}
    \varphi(y +\gamma x) - \varphi(y) > \varphi(y + \gamma x - \tilde{x}) -
    \varphi \ge 0.
\end{align}
Thus given a local minimum of the relaxed optimization problem we can find
...
\tilde{x}$) is a local minimum of the optimization problem. Thereby any
...
\begin{align}
    ...
    \end{cases},
\end{align}
is a KKT point and then $x$ is a local minimum.
Here $L:
...
When trading trade with multiple DEXs, the agent will
interacts with multiple CFMMs. In light of multi asset trades
...
\{0,1\}^{m} $. What we have here is a mixed integer convex optimization
problem. A possible solution might evolve the relaxation to the condition
...
equation \ref{eq: multi_dex_trade}. It is important to note here that the
solution does specify in which order the trades should be executed and that
the computational effort will grow larger with more CFMM. Hence, in a large
space of CFMM it is a separate problem of finding cyclic arbitrage path. This
path will give the order of the trades and specify for which CFMM the
...
valid, i.e. $\sum_{i=1}^{m} A_i(\tilde{x}_i-x_i) \neq 0$. The agent also needs to have $\sum_{i=1}^{m} A_i(\tilde{x}_i-x_i) \ge 0$, since
at the end of a set of trades, the arbitrage trade receives a positive amount
of at least one asset. This hold only for $\sum_{i=1}^{m} A_i(\tilde{x}_i-x_i)$, i.e. at the end of the
...
i.e. $U\left(\sum_{i=1}^{m} A_i(\tilde{x}^{*} _i-x^{*} _i)\right) > U(0)$
with $x^{*}, \tilde{x}^{*} $ the optimal solution. As discussed we have the
follwing optimization problem for the general multiasset optimal trade
\begin{align}
