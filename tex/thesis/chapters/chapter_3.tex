\chapter{Constant Function Market Makers\label{chap: cfmms}}
On top of blockchain technology, a new form of financial markets has found
its roots, they are called Decentralized Exchanges (DEXs). As in any
financial market, DEXs allow agents to trade cryptocurrencies, which we will
refer to throughout as an asset. Thorough a decentralized ledger, stored and copied
on multiple computers (blockchain), the asset ownership is being tracked
by a storage slot of a smart contract. All agents holding an asset have
an entry in the storage slot, equivalent to a database entry in the
asset's smart contract with their wallet address and an amount. In this
chapter we go through the underlying principles of DEXs and their properties
by mathematical modeling and optimization to arrive at optimal trades and
ultimately model arbitrage scenarios. In recent years a lot of groundwork on
modeling has been done in \cite{cfmms_basics} and on optimal trades in
\cite{optimal_routing_cfmm}, on which this chapter is based on. We give an
extensive description in a specific case of optimal arbitrage in a network of
CFMMs all based on \cite{uniswap} and \cite{uniswapv3_paper}.
...
and a set of conditions. The conditions define the way an agent interacts
with a DEX. Here $n \ge 2$ is the number of assets an agent can trade, these
are labeled $\{1,\ldots, n\}$. The CFMM is a function of the reserves $y \in
\mathbb{R}^{n}_+$, where $y_i \in \mathbb{R}_+$ is the reserve of asset $i
\in \{1,\ldots,n\}$. The reserves are the liquidity of the DEX and generally
lie in its smart contract portfolio. DEXs are also sometimes referred to
simply as liquidity pools. It might sound confusing but the terms DEX, CFMM
and liquidity pool can be used interchangeably. There are two underlying ways
an agent can interact with the DEX:
\begin{itemize}
    \item Change liquidity
    \item Trade
\end{itemize}
We will only look at the trading interaction which lies in the scope of the
thesis.
\newline
...
given and received asset $i \in \{1,\ldots,n\}$ respectively. A trade can either
be accepted or rejected, depending on an acceptance principle of the
underlying CFMM. If the trade is accepted the state of the pool is updated by
changing the reserves
\begin{align}
    y \mapsto y + x - \tilde{x}.
\end{align}
A necessary condition for trade acceptance is that $y \ge 0$ after the trade.
In the case the trade is rejected the reserves do not change. Throughout we
assume that the vectors $x$ and $\tilde{x}$ have disjoint support. This is a
valid assumption, since when trading, the agent is never giving and receiving
the same asset, but rather one or multiple different ones. The support of a
vector $v \in \mathbb{R}^{n}_+$ is $\text{supp}(v) := \{i\; |\; v_i > 0\}$,
thus the assumption is $\text{supp}(x)\;\cap\; \text{supp}(\tilde{x}) =
\emptyset$.
...
\end{align}
Depending on the CFMM, a trade has a fee $\gamma \in (0, 1]$. Then
$(1-\gamma)x$ of the set amount of given assets is distributed to liquidity
providers as a compensation for providing liquidity. The discounted amount
...
$|\text{supp}(x)| = |\text{supp}(\tilde{x})| = 1$. Let $i$ and $j$ be the
corresponding indices of the giving and receiving asset then an analytic
expression can be explicitly derived  for $x_i$ (or $\tilde{x}_j$) by taking
the power of $w_i$ (or $w_j$) and rearranging equation
\ref{eq: wm_trade_acceptance} gives
\begin{align}
...
Curve \cite{stableswap} market makers use the following trading function
\begin{align}
    \varphi(y) =\sum_{i=1}^{n}y_i  - \alpha \prod_{i=1}^{n} y_i^{-1} ,
\end{align}
where $\alpha > 0$. This trading function is used for stable coin markets to
reduce price impact without large counter liquidity. An aggregated
generalization is a combination between the geometric weighted mean and the
(un)weighted sum is
\begin{align}
    \varphi(y) =(1-\alpha)\sum_{i=1}^{n}y_i  - \alpha \prod_{i=1}^{n} y_i^{w_i},
\end{align}
where $\sum_{i=1}^{n}w_i  = 1$, $w_i > 0$ and $\alpha \in [0,1]$. For $\alpha
= 0$, the trading function becomes the negative weighted sum and for $\alpha
= 1$ the geometric weighted mean.

\subsection{Concentrated Liquidity}
Another implementation of AMMs are CFMMs with concentrated liquidity like
UniswapV3 \cite{uniswapv3_paper}. The idea is to split liquidity provision
from the whole sublevel set $\varphi(y +\gamma x-\tilde{x}) =\varphi(y)$ to
multiple smaller sets. The sets are determined by ticks $\{p(i)\}_{i \in
\mathbb{Z} }$ with the rate usually set to $p(i) = 1.0001^{i}$, where $p(i)$
is the so-called marginal exchange rate of the tick $i$. Every tick range
\begin{align}
    (p(i), p(i+1)],
\end{align}
has its own reserves (liquidity) and  all of them are governed by a single
trading function $\varphi$ depending on the liquidity bound by the tick
range. The trade is executed in a specific tick range when the marginal
exchange rate is in the this specific tick range. Then the liquidity of this
specific tick range is used for the trade.

The liquidity providers specify an upper and a lower bound $(p(U), p(L)]$ and
provide liquidity on a depth $\tilde{k} = \varphi(y)$. They earn the fee of
the trades, which is split by the weighted amount of liquidity provided in
the tick range by each liquidity provider instead of on the whole trading
curve.

When the marginal exchange rate raises above a trick range during the trade,
...
For fix reserves $y \in \mathbb{R}_+^{n}$ let $\nabla \varphi(y) = P$ be the
vector of unscaled prices. Note that $P$ is positive in all entries because
$\varphi$ is by assumption increasing. The entries are given by the gradient
\begin{align}
...
\begin{align}
    p_i = \frac{P_i}{P_n}, \label{eq: marginal_rate}
\end{align}
where $p_n = 1$ always.

Recall the trade acceptance condition \ref{eq: trade_acceptance_condition},
...
\end{align}
also holds.
Note that this is a first order approximation, similarly to a midpoint price
in an order book. The actual trades are completely determined by the trade
acceptance condition. It is an reassuring fact, that the trade can
approximated by the marginal exchange rates which is the usual intuition. We
will later see, that by concavity of the trade function this will hold in an
inequality.
\section{Swaps}
The most common type of trade interaction with a DEX is a swap, i.e. trading
one asset for another. Denote these two assets with index $i$ and index $j$,
...
It is natural to define a function $F: \mathbb{R}_+ \to \mathbb{R}$, the
forward exchange function. Where $F(\delta)$ is unique to one
$\tilde{\delta}$. The function $F$ is defined for all valid trades.
Additionally, it increasing since $\varphi$ is increasing. Also, $F$ is
non-negative, because $F(0) = 0$ and it is concave. To show concavity we use
the implicit function theorem on the trade acceptance condition
\begin{align}
f(\delta, \tilde{\delta}) = \varphi(y + \gamma\delta e_i -\tilde{\delta}e_j)
- \varphi(y) =0,
\end{align}
denote $y' = y+\gamma\delta e_i + \tilde{\delta}e_j$. The chain rule gives us
\begin{align}
...
\end{align}
holds for all valid $\delta, \delta' \ge 0$. Let $y'' = y + \gamma \delta'
e_i - F(\delta') e_j$ and note that $\varphi(y) = \varphi(y') = \varphi(y'')$
as part of the trade acceptance condition. Since $\varphi$ is a concave
function
\begin{align}
    \varphi(y'') \le \nabla\varphi(y')^{T} (y'' -y')+
...
Respectively we define the reverse exchange function $G: \mathbb{R}_+
...
function is non-negative and increasing because of $\varphi$, but unlike
the forward exchange function it is convex. To show convexity, we can again
use the implicit function theorem and apply a similar argumentation to the
forward exchange function, i.e.
\begin{align}
...
\end{align}
for all valid $\tilde{\delta},\tilde{\delta}' \ge 0$. We set $y'' = y + \gamma
G(\tilde{\delta}')e_i - \tilde{\delta}'e_j$  and use the concavity of the
trading function $\varphi$ together with the trade acceptance condition to
get
\begin{align}
    &\nabla \varphi(y')^{T} (y'' - y') \ge 0\\
    ...
    + G(\tilde{\delta})
\end{align}
For two asset trades on the geometric weighted mean trading function in equation
...
\right),
\end{align}
for $\tilde{\delta} \le y_j$ or else $G(\tilde{\delta}) = \infty$ (invalid
trade).
...
exchange function we can compute the mapping numerically. Since the trade is
uniquely defined by one variable either $\delta$ or $\tilde{\delta}$, we can
fix one to compute a solution to the trade acceptance condition numerically
\ref{eq: trade_acceptance_condition}. Recall the Newton method, for a
continuously differentiable function $f$ with an initial guess
$x_0$ in an adequate neighborhood of the root, with the iteration
\begin{align}
...
-\tilde{\delta}_k e_j)}.
\end{align}
with optimal values at $\delta^{*}, \tilde{\delta}^{*}$ repsectively.
...
the derivative. Assuming $\varphi$ is continuously differentiable, the
...
+\gamma \delta e_i -\tilde{\delta}e_j) - \varphi(y)$. Then we can set
\begin{align}
...
$\varepsilon$ around the solution, and also that
\begin{align}
...
\begin{align}
    ...
    |f'(\tilde{\delta}^{*})|
\end{align}
holds for all $\tilde{\delta}$ in the epsilon ball of the solution. Then by
the mean value theorem and Lipschitz continuity for all $\tilde{\delta}$ in
the $\varepsilon$ ball around the solution we have
\begin{align}
    ...
    +|f'(\tilde{\delta}) - f'(\tilde{\delta}^{*})|
    |\tilde{\delta}-\tilde{\delta}^{*} | \\
    &\le \frac{1}{4c} |\tilde{\delta} - \tilde{\delta}^{*}| + \frac{1}{2c}
    |\tilde{\delta} - \tilde{\delta}^{*}| = \frac{3}{4c} |\tilde{\delta} -
    \tilde{\delta}^{*}|.
\end{align}
...
\begin{align}
    ...
    &\le \frac{3}{4} |\tilde{\delta}_0-\tilde{\delta}^{*} |
\end{align}
Repeating this argument $k\ge0$ times we get
\begin{align}
    |\tilde{\delta}_{k+1}-\tilde{\delta}^{*} | \le \left( \frac{3}{4} \right)^{k}
    |\tilde{\delta}_0-\tilde{\delta}^{*}|
\end{align}
Thus the iterates of the Newton method $(\tilde{\delta}_k)_{k\in\mathbb{N}}$
lie in the $\varepsilon$ ball of the solution, the iteration is well defined
since the derivatives are non zero and the iterates converge to
$\tilde{\delta}^{*}$ as $k\to\infty$. To solve for $\delta^{*} $, fix
$\tilde{\delta}$, the proof stays the same.

\section{General \& Optimal Trades on one CFMM}
For general trades, the agent chooses a trade $(x, \tilde{x})$ through the
solution of an optimization problem. The optimization problem maximizes the
change in the traders' portfolio $x -\tilde{x}$ through a utility function.
The agent prefers a change in holdings that maximize a utility function $U:
\mathbb{R}^{n}  \to \mathbb{R} \cup \{-\infty\} $. For two trades $(x,
\tilde{x}$ and $(x',
\tilde{x}')$ the agent prefers the trade $(x, \tilde{x})$ if
\begin{align}
    U(\tilde{x} -x) > U(\tilde{x}' - x').
\end{align}
We map $U$ to  $\{-\infty\}$ when the trade does not pass the trade
acceptance condition or the change in the portfolio is absolutely not
preferred. We choose the utility function to be increasing, concave, and
thereby differentiable. By the trade acceptance condition, the trade that
maximizes the utility can be framed as a constrained optimization problem
\begin{align}
...
\end{align}
We would prefer the problem in equation \ref{eq: gen_trade} to be convex.
Then it can be solved efficiently fast using convex methods. The feasible set
...
\end{align}
cannot be reduced further, thus the convex combination generally does belong
to $D$ . But, if we consider the relaxation from equality in $D$ to inequality
$\ge$, i.e.
\begin{align}
...
we can reduce the equation \ref{eq: convexity_trade_condition} further.
\begin{align}
    &\varphi\Big(y + \lambda(\gamma x_1 - \tilde{x}_1) + (1-\lambda)(\gamma
...
\begin{align}
    ...
    &\lambda \left(\varphi(y + \gamma x_1 - \tilde{x}_1) - \varphi(y)\right)
    + (1-\lambda)\left(\varphi(y+\gamma x_2 - \tilde{x}_2) -
    \varphi(y)\right)\ge 0,
\end{align}
for all $\lambda \in [0, 1]$. Hence, $D^{r}$ is convex. So the relaxation of
problem \ref{eq: gen_trade}, i.e.
\begin{align}
...
than that of $(x,\tilde{x})$. Thus, $(x,\tilde{x})$ is not an optimal
solution.
\newline

Furthermore the relaxed problem fulfills Slater CQ. By the choice of
$\tilde{x}=0$ we can choose any $x \in \mathbb{R}^{n}_+$ such that by the
monotonicity assumptions on $\varphi$ we have
\begin{align}
    \varphi(y +\gamma x) - \varphi(y) > \varphi(y + \gamma x - \tilde{x}) -
    \varphi(y) \ge 0.
\end{align}
Thereby, given a local minimum of the relaxed optimization problem we can find
Lagrange multipliers, forming a KKT-point satisfying the KKT conditions
\ref{def: KKT}. And on the other hand if we find a KKT point we will have a
local minimum.
...
\end{align}
Since Slater CQ is satisfied so is Abadie CQ, given some local minimum then
there is a KKT point $(x , \tilde{x}, \lambda , \omega , \mu)$, which is a
sufficient condition for optimality in the convex case with Slater CQ. Hence, $(x
, \tilde{x}$) is a local minimum of the optimization problem and any
$(x, \tilde{x}, \lambda, \omega, \mu)$ satisfying
\begin{align}
    \begin{cases}
...
\to \mathbb{R} $ is the Lagrangian, $\lambda \in \mathbb{R}_+, \omega \in
\mathbb{R}_+^{n}, \mu \in \mathbb{R}_+^{n}$  are the Lagrange multipliers and
$g(x, \tilde{x}) := \varphi(y) - \varphi(y + \gamma x - \tilde{x})$ a
notational convention. The Lagrangian of the problem is
\begin{align}
...
\begin{align}
    ...
    \nabla\varphi(y').
\end{align}
Specific interpretation for this condition comes when looking the trade that
is not increasing the utility, e.g. for a solution $x = \tilde{x} = 0$. Then
the condition, after dividing with $\lambda P_n$ becomes
\begin{align}
    \gamma p \le \frac{1}{\lambda P_n}\nabla U(0) \le p.
\end{align}
The set of prices $p$ which satisfy this condition is denoted by
\begin{align}
    K := \left\{p \in \mathbb{R}_+^{n}| \;\exists \alpha\ge0:\; \gamma p \le
    \alpha U(0) \le p \right\},
\end{align}
in literature referred to as the no-trade cone for the utility U of trade
...
In the case of linear utility $U(\psi) = z^{T} \psi$, with $z \in
\mathbb{R}_+^{n}$ we have that
\begin{align}
    \gamma p \le \frac{1}{\lambda P_n}z \le p,
\end{align}
hods component wise.
\section{Multi-DEX Trades}
When trading with multiple DEXs, the agent interacts with multiple CFMMs. In
light of multi asset trades \ref{eq: convex_gen_trade} a generalization to an
interaction with multiple CFMMS can be made. Consider assets $j \in \{1,
\ldots,n\}$, labeled as such. Consider CFMMs $i \in \{1,\ldots,m\}$, each
trading $N_i \subseteq \{1,\ldots,n\}  $ assets which are specifically
ordered based on the construction of the CFMM $i$ and trading function
$\varphi_i: \mathbb{R}^{|N_i|}_+\to \mathbb{R}$ with liquidity $y_i$ and
trading fee $\gamma_i$. To link the ordering of $N_i$ and asset labels $i$,
consider the binary matrix $A_i \in \mathbb{R}^{n\times |N_i|}$ with
\begin{align}
    (A_i)_{kj} =
    \begin{cases}
...
The agent interacts with each CFMM $i$ through a trade $(x_i, \tilde{x}_i)$, the
...
\end{align}
The implication of $\psi_k \ge 0$, means that the agent does not give any
amount of asset $k$ at the end of the trade to the set of CFMMs. The agent
still might trade asset $k$, but receives a positive amount. Analogous to
multi-asset trades, the agent judges a trade with a utility function
\begin{align}
...
the agent wants to increase the holdings $h\in \mathbb{R}_+^{n}$ after the
...
A set of optimal trades with multiple CFMMs, maximizing the utility is
\begin{align}
    \text{max} \quad &U\left(\sum_{i=1}^{m} A_i(\tilde{x}_i-x_i)\right)\\
    \text{s.t.:}\quad
...
component-wise for all CFMMs $i \in \{1,\ldots,m\}$. The convex relaxation of
the problem is
\begin{align}
...
setting $\tilde{x}_i = 0$. Since the problem is convex and Slater CQ is
satisfied the KKT conditions are necessary and sufficient for optimality. The
...
\begin{align}
    ...
    \Rightarrow& \nabla U(x, \tilde{x}) = \nu.
\end{align}
Also for all $i \in \{1,\ldots,m\}$ we have
\begin{align}
...
called during the transaction. In light of this we let $q_i \in \mathbb{R}_+$
be some cost related to executing a trade with CFMM $i$. The cost $q_i$ is only
payed if the transaction is executed (or partially executed), i.e. $\tilde{x}_i -
...
\{0,1\}^{m} $. This is a mixed integer convex optimization
problem. A possible solution might evolve the relaxation of the condition
$\eta \in [0, 1]^{m}$ to transform it to a convex optimization problem. And
introduce a learned parameter $\tau(\theta)$ with some weights $\theta$, as a
threshold to flip the value of $\eta$ to one or zero. This however would
evolve learning $\tau$, by solving the mixed integer problem many times in
real world scenarios. The trade-off would be to have a faster solution when
computing in real time.
\section{Cyclic Arbitrage}
Let us go back to the general trading problem without transaction fees in
equation \ref{eq: multi_dex_trade}. It is important to note, that the
solution does specify in which order the trades should be executed and that
the computational effort will grow larger with more CFMMs. Hence, in a large
space of CFMMs it is a separate problem of finding cyclic arbitrage walks. This
walk will give the order of the trades and specify for which CFMM the
optimization problem needs to be framed, dropping the rest.
\newline

We consider now that the agent has found a cyclic arbitrage path of $m \in
\mathbb{N} $ CFMMs. For an arbitrage trade, the agent needs the trade to be
valid, i.e. $\sum_{i=1}^{m} A_i(\tilde{x}_i-x_i) \neq 0$. The agent also
needs $\sum_{i=1}^{m} A_i(\tilde{x}_i-x_i) \ge 0$, because at the end
of a set of trades, the arbitrage trade receives a positive amount of at
least one asset. This hold only for $\sum_{i=1}^{m} A_i(\tilde{x}_i-x_i)$,
i.e. at the end of the trade. The utility function needs to be bound in the
domain $\mathbb{R}_+^{n}$. There is an arbitrage, if there is a nonzero
solution. An optimal arbitrage is one that maximizes the utility under the
constraints i.e. $U\left(\sum_{i=1}^{m} A_i(\tilde{x}^{*} _i-x^{*} _i)\right)
> U(0)$ with $x^{*}, \tilde{x}^{*} $ the optimal solution. As discussed we
have the follwing optimization problem for the general multiasset optimal
trade
\begin{align}
...
\subsection{Arbitrage on two Asset Constant Product Market Makers}
...
1\\ 0 & 0\end{pmatrix}$. Then the constraint can be written into quadratic
form
\begin{align}
...
\end{align}
Additionally, it is possible use matrix factorization to factor
$\sum_{i=1}^{n}A_iR\xi_i$ into $A\xi$ for $A \in \mathbb{R}^{n\times2n}$ with
entries $-1, 0, 1$, together with $\xi = \begin{pmatrix} \delta_1 &
\tilde{\delta}_1 & \hdots & \delta_n & \tilde{\delta}_n \end{pmatrix}^{T} \in
\mathbb{R}^{2n}  $. We can also pull the inequality constraint into a single
constraints under the same $\xi$ with a $c \in \mathbb{R}^{2n}$ and $D \in
\mathbb{R}^{2n \times 2n}$ consisting of $n$ blocks of $\gamma_iB$, to get
\begin{align}
