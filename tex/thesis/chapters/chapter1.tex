...
\begin{align}
    Ad(\tau) = Ad + \tau Ad' = 0.
\end{align}
We can show that $d(\tau) \in T_X(x^{*})$ for all $\tau >0 $, which concludes
the proof by the closure of $T_X(x^{*})$. Let
\begin{align}
    x^{k}  = x^{*} + \frac{1}{k}d(\tau),\\
    t_k = 1/k,
\end{align}
...
\begin{align}
    \frac{x^{k} - x^{*}  }{t_k} = d(\tau)
\end{align}
To finish the proof, we need to show that $x^{k}  \in X$, the feasible set,
i.e. we show that $g_i(x^{k}) \le 0$ for all $i=1,\ldots,m$ and $Ax^{k} -b =
...
segment of $x^{*}$ and $x^{k}$ sucht that
\begin{align}
    g_i(x^{k})
    &= g_i(x^{k}) - g_i(x^{*}) \\
...
\end{align}
Taking $k_0 = \max{k_0^{i} , k_1^{i}}$ over all indices $i = 1,\ldots,m$
