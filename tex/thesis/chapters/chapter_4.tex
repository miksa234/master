...
\end{align}
Here $\|$ is the concatenation operator,  $w_{ij}$ is the weight of the edge
(or edge attribute), $\text{MLP}^{k}$ is a $k$-th layer multilayer
perceptron, where the last weight can produce negative values, $h_i^{(k)}$
and $h_j^{(k)}$ is the node feature embeddings after the $k$-th GNN layer.
The input features are in the 0-th layer $h_i^{(0)} = x_i$. Then the node
embedding for the next layer is calculated by
\begin{align}
...
\begin{align}
    ...
    \right],
\end{align}
where $G_t$ is the sum returns from $t+1$ to the terminal state $T$. The
...
TODO: Move to next section remove NP-hard
