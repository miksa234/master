...
large counter liquidity. An aggregated generalization is a convex combination between
...
\begin{align}
    ...
    \tilde{\delta})}{\partial_{\tilde{\delta}} f(\delta, \tilde{\delta})}\\
       ...
       \varphi(y') \right)_j} \label{eq: forward_implicit}
\end{align}
...
\begin{align}
    ...
    F(\tilde{\delta}) = \frac{1}{\gamma} \frac{p_j}{p_i},
\end{align}
for $\tilde{\delta} \le y_j$ and else $G(\tilde{\delta}) = \infty$ (invalid
trade).
...
Recall the Newton-Raphson method for a continuously differentiable function in
...
the iterations
\begin{align}
    \delta_{n+1} = \delta_n - \frac{\varphi(y+\gamma\delta_n e_i
    -\tilde{\delta}e_j) -\varphi(y)}{\partial_{y_i}\varphi(y+\gamma\delta_n e_i
-\tilde{\delta}e_j)}\label{eq: iter_backward},\\
\tilde{\delta}_{n+1} = \tilde{\delta}_n + \frac{\varphi(y+\gamma\delta e_i
    -\tilde{\delta}_ne_j) -\varphi(y)}{\partial_{y_j}\varphi(y+\gamma\delta e_i
-\tilde{\delta}_n e_j)}.
\end{align}
An adequate choice for $\delta_0$ and $\tilde{\delta}_0$ can be deduced from
the first order approximation in section \ref{sec: first_order_approx}.
\begin{align}
    \delta_0 = \frac{1}{\gamma}\frac{p_j}{p_i}\tilde{\delta}, \qquad
    \tilde{\delta}_0 = \gamma\frac{p_i}{p_j}\delta.
\end{align}
The convergence of $\tilde{\delta}_n$ to $\tilde{\delta}^{*}$ is linearly
decreasing, let $y_n := y + \gamma \delta e_i - \tilde{\delta}e_j$ then
\begin{align}
    |\tilde{\delta}_{n+1} - \tilde{\delta}^{*} | &\le
    \left| \frac{\varphi (y_n) -\varphi(y)}{\partial_{y_j}\varphi(y_n)}
    +\tilde{\delta}_n  - \tilde{\delta}^{*} \right|\\
    &\le
    \left| \frac{\varphi (y_n) -\varphi(y)}{\partial_{y_j}\varphi(y_n)}
    \right| + |\tilde{\delta}_n  - \tilde{\delta}^{*} |,
\end{align}
since $\varphi$ is concave, using equation \ref{eq: phi_concave} simplifies
to
\begin{align}
    |\tilde{\delta}_{n+1} - \tilde{\delta}^{*} | &\le
    \left| \frac{\nabla\varphi(y_n)^{T}(y_n-y)}{\partial_{y_j}\varphi(y_n)}
    \right| + |\tilde{\delta}_n  - \tilde{\delta}^{*} |\\
    &\le
    \left|
    \frac{\partial_{y_i}\varphi(y_n)}{\partial_{y_j}\varphi(y_n)}\gamma\delta
    -
    \frac{\partial_{y_j}\varphi(y_n)}{\partial_{y_j}\varphi(y_n)}\tilde{\delta}_n
    \right| + |\tilde{\delta}_n  - \tilde{\delta}^{*} |.
\end{align}
Recall the concavity of $F(\delta)$ for valid $\delta$, then by the implicit
function theorem equation \ref{eq: forward_implicit} together with the
concave property of $F$ and setting $\delta' = 0$ the above simplifies to
\begin{align}
    \tilde{\delta} = F(\delta) \le F'(0) \delta = \frac{\partial_{y_i}
    \varphi(y')}{\partial_{y_j} \varphi(y')} \gamma \delta
\end{align}
hence
\begin{align}
    \tilde{\delta} -
    ...
    \le 0.
\end{align}
Back to the convergence rate to get
\begin{align}
    |\tilde{\delta}_{n+1} - \tilde{\delta}^{*} | &\le
    |\tilde{\delta}_{n} - \tilde{\delta}^{*} | \\
&\le |\tilde{\delta}_{0} - \tilde{\delta}^{*} |
\end{align}
Similarly for the sequence $(\delta_n)_{n\in\mathbb{N} }$ in equation
\ref{eq: iter_backward}, with $\delta^{*}$ optimal by
\begin{align}
    |\delta_{n+1} - \delta^{*} | &\le
    \left | \delta_n - \frac{\varphi(y') - \varphi(y)}{\partial_{y_i}
    \varphi(y')} - \delta^{*}  \right | \\
    &\le
    \left | - \frac{\varphi(y') - \varphi(y)}{\partial_{y_i}
    \varphi(y')} \right| + \left| \delta_n - \delta^{*}  \right |. \\
\end{align}
Using the convexity of $G$
\begin{align}
    G(\tilde{\delta}) \ge G'(0) \tilde{\delta}\\
    \frac{\partial_{y_j}\varphi(y')}{\partial_{y_i}\varphi(y')}\tilde{\delta}
    - \gamma\delta \le 0,
\end{align}
and the concavity of $\varphi$ to arrive at
\begin{align}
    |\delta_{n+1} - \delta^{*} | &\le
    |\delta_{0} - \delta^{*} |.
\end{align}
...
\begin{align}
    K := \left\{p \in \mathbb{R}_+^{n}| \; \gamma p \le \alpha U(0) \le p,\;\;
    \alpha \ge 0  \right\}
\end{align}
...
\begin{align}
    \text{max} \quad &U(\psi) \label{eq: multi_dex_trade_orig}\\
    \text{s.t.:}\quad & \psi = \sum_{i=1}^{m} A_i(\tilde{x}_i-x_i)\\
...
problem is
\begin{align}
    \text{max} \quad &U(\psi) \label{eq: multi_dex_trade}\\
    \text{s.t.:}\quad & \psi = \sum_{i=1}^{m} A_i(\tilde{x}_i-x_i)\\
          & (x_i, \tilde{x}_i) \in D_i^{r}
          \;\; \forall i \in
          \{1,\ldots,m\},
\end{align}
where
...
Thus, the trade cones are defined by the
\begin{align}
...
\begin{align}
    \text{max} \quad &U(\psi) - q^{T}\eta  \label{eq: trade_transaction_cost}\\
    \text{s.t.:}\quad & \psi = \sum_{i=1}^{m} A_i(\tilde{x}_i-x_i)\\
          & (x_i, \tilde{x}_i) \in D_i^{r}\\
          &x_i \le \eta_i x_i^{*}, \quad \forall i \in\{1,\ldots,m\} \\
          &\eta_i \in \{0, 1\},
\end{align}
with variables $\psi, x_i, \tilde{x}_i$, $i\in\{1,\ldots,m\}$ and $\eta \in
...
\begin{align}
    \text{max} \quad &U(\psi) \label{eq: arb_trade}\\
    \text{s.t.:}\quad & \psi = \sum_{i=1}^{m} A_i(\tilde{x}_i-x_i)\\
          & \psi \ge 0\\
          & (x_i, \tilde{x}_i) \in D_i^{r}
          \;\; \forall i \in
          \{1,\ldots,m\},
\end{align}
Again the solution $\psi=0$ is valid, but in this case there is no arbitrage
and the agent does not trade.
\subsubsection{Example}
A common case is an $n$ cyclic arbitrage under linear utility $U(\psi)
=1^{T}\psi$  with $n$ CFMMs $\varphi_i(y_i) =
...
\begin{align}
   ...
   1 & 0
    \end{pmatrix}.
\end{align}
Also, the general trade vector $\psi \in \mathbb{R}^{n}$ equality is
\begin{align}
    \psi =
    \begin{pmatrix}
...
\begin{align}
    \text{max} \quad & 1^{T}\psi \label{eq: quadratically_constrained}\\
    \text{s.t.:}\quad & \psi = \sum_{i=1}^{n} A_iR\xi_i\\
          & \psi \ge 0 \;\; \xi_i \ge 0\\
          & c_i^{T} \xi_i + \gamma_i \xi_i^{T} B\xi_i \le 0
          \;\; \forall i \in \{1,\ldots,n\}.
\end{align}
Additionally, it is possible to transform $\psi = \sum_{i=1}^{n}A_iR\xi_i$
into $\psi = A\xi$ for $A \in \mathbb{R}^{n\times2n}$ with entries $-1, 0,
1$, with $\xi = \begin{pmatrix} \delta_1 & \tilde{\delta}_1 & \hdots &
\delta_n & \tilde{\delta}_n \end{pmatrix}^{T}  \in \mathbb{R}^{2n}  $. Also,
it is possible to pull the inequality constraint into a single constraint
under the same $\xi$ with a $c \in \mathbb{R}^{2n}$ and $D \in \mathbb{R}^{2n
\times 2n}$ consisting of $n$ blocks of $\gamma_iB$, to get
\begin{align}
    \text{max} \quad & 1^{T}\psi \label{eq: quadratically_constrained_general}\\
    \text{s.t.:}\quad & \psi - A\xi = 0\\
          & c^{T} \xi + \xi^{T} D\xi \le 0\\
          & \psi \ge 0 \;\; \xi \ge 0.
\end{align}
