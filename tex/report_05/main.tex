...
\subsubsection{Optimality Condition\label{sec: optimality condition}}
...
A common case is an $n$ cyclic arbitrage under linear utility $U(x, \tilde{x})
=1^{T}\left(\sum_{i=1}^{m} A_i(\tilde{x}_i-x_i)\right)$  with $n$ CFMMs $\varphi_i(y_i) =
...
\begin{align}
    ...
    \text{s.t.:}\quad & c^{T} \xi + \xi^{T} D\xi \le 0\\
          & \xi \ge 0.
\end{align}
