...
\begin{figure}[htb!]
    ...
    \begin{tikzpicture}[scale=0.8, transform shape]
        ...
        \node[align=center, draw, ultra thick, below=of 2] (3)
            {current state = child node \\
            that maximizes $UCB1(s_k)$?};

        \node[align=center, draw, ultra thick, right=2cm of 2] (4)
            {Was the current state\\ visited before ($n_k > 0$)};

        \node[align=center, ellipse, draw, ultra thick, above=of 4] (5)
            {Roll-out};
        ...
        \node[align=center, draw, ultra thick, below=of 6] (7)
            {current state = first new child node};

        \node[align=center, draw, ultra thick, below=of 7] (8)
            {Roll-out};
        ...
        \draw[->, ultra thick] (4) to node[midway, right] {YES} (5);
...
pair is computed via a function $a_{ij} = a(x_i, x_j)$, usually a
...
\begin{align}
    a_{ij}^{k} = \text{sign}\left(e_{ij}  \right) \cdot
...
\end{align}
Here $w_ij$ is the weight of the edge, $\text{MLP}^{k}$ is a k-layer
multilayer perceptron, where the last weight can produce negative values,
$h_i^{k}$ and $h_j^{k}$ is the node feature embeddings after the $k$-th GNN
layer. The input features are in the 0-th layer $h_i^{0} = x_i$.
...
reward $R$ must be assigned correspondingly. It makes since to keep a track
...
make since to somehow encode the valid moves in the input through dynamic
edges, i.e. removing edges. But will the removal of the edges even make since
from a reinforcement learning perspective ? How should the input layer look
like ? The code can be found in \cite{github}.
